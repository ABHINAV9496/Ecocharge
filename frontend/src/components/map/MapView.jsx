    ...
    <Marker position={[lat, lng]} icon={createStationIcon(mStatus, false)}>
      <Popup><div className="min-w-[220px]">
        ...
        <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400 mb-2.5">
          <span className="flex items-center gap-1"><FiZap className="w-3 h-3" />{station.slots ? station.slots.filter(function (s) { return s.available !== undefined ? s.available : s.status === 'AVAILABLE' }).length : 0} free</span>
        </div>
...
export default function MapView({ routePlan }) {
  ...
  var filteredStations = useMemo(function () {
    return stations.filter(function (station) {
      ...
      if (statusFilter === 'available') passesStatus = station.slots && station.slots.some(function (s) { return s.available !== undefined ? s.available : s.status === 'AVAILABLE' })
