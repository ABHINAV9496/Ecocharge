...
export default function StationDetailPage() {
  ...
  function isAvailable(s) { return s.available !== undefined ? s.available : s.status === 'AVAILABLE' }

  var availableCount = slots.filter(isAvailable).length
  var slotGroups = {}
                      ...
                      <div key={groupType}>
                        ...
                        <div className="space-y-2">
                          {groupSlots.map(function (slot) {
                            var slotAvailable = isAvailable(slot)
                            var statusColor = slotAvailable ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50/80 dark:bg-emerald-900/20' : slot.status === 'FAULT' ? 'border-red-200 dark:border-red-800 bg-red-50/80 dark:bg-red-900/20' : 'border-amber-200 dark:border-amber-800 bg-amber-50/80 dark:bg-amber-900/20'
                            var statusBadge = slotAvailable ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400' : slot.status === 'FAULT' ? 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400' : 'bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400'
                              ...
                              <div key={slot.id} className={'p-3.5 border rounded-xl transition-all ' + statusColor}>
                                <div className="flex items-center justify-between mb-2">
                                  <div className="flex items-center gap-2">
                                   ...
                                   <span className={'px-2 py-0.5 text-xs font-medium rounded-full ' + statusBadge}>{slotAvailable ? 'Available' : slot.status === 'FAULT' ? 'Offline' : 'In Use'}</span>
                                </div>
